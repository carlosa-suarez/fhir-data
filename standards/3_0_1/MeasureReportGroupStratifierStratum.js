...
class MeasureReportGroupStratifierStratum extends BackboneElement {
	...
	set population (new_value) {
		...
		this.__population = Array.isArray(new_value) ? new_value.map(val => new MeasureReportGroupStratifierStratumPopulation(val)) : [new MeasureReportGroupStratifierStratumPopulation(new_value)];
	}
