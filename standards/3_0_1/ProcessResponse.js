...
class ProcessResponse extends DomainResource {
	...
	set processNote (new_value) {
		...
		this.__processNote = Array.isArray(new_value) ? new_value.map(val => new ProcessResponseProcessNote(val)) : [new ProcessResponseProcessNote(new_value)];
	}
	...
	set error (new_value) {
		const CodeableConcept = require('./CodeableConcept');
		this.__error = Array.isArray(new_value) ? new_value.map(val => new CodeableConcept(val)) : [new CodeableConcept(new_value)];
	}
