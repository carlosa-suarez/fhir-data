...
class EpisodeOfCare extends DomainResource {
	...
	set statusHistory (new_value) {
		...
		this.__statusHistory = Array.isArray(new_value) ? new_value.map(val => new EpisodeOfCareStatusHistory(val)) : [new EpisodeOfCareStatusHistory(new_value)];
	}
	...
	set diagnosis (new_value) {
		...
		this.__diagnosis = Array.isArray(new_value) ? new_value.map(val => new EpisodeOfCareDiagnosis(val)) : [new EpisodeOfCareDiagnosis(new_value)];
	}
	...
	set referralRequest (new_value) {
		const Reference = require('./Reference');
		this.__referralRequest = Array.isArray(new_value) ? new_value.map(val => new Reference(val)) : [new Reference(new_value)];
	}
