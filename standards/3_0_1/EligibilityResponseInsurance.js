...
class EligibilityResponseInsurance extends BackboneElement {
	...
	set benefitBalance (new_value) {
		...
		this.__benefitBalance = Array.isArray(new_value) ? new_value.map(val => new EligibilityResponseInsuranceBenefitBalance(val)) : [new EligibilityResponseInsuranceBenefitBalance(new_value)];
	}
