...
class TestScriptSetupActionAssertRulesetRule extends BackboneElement {
	...
	set param (new_value) {
		...
		this.__param = Array.isArray(new_value) ? new_value.map(val => new TestScriptSetupActionAssertRulesetRuleParam(val)) : [new TestScriptSetupActionAssertRulesetRuleParam(new_value)];
	}
