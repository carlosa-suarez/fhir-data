...
class ElementDefinitionSlicing extends Element {
	...
	set discriminator (new_value) {
		const ElementDefinitionSlicingDiscriminator = require('./ElementDefinitionSlicingDiscriminator');
		this.__discriminator = Array.isArray(new_value) ? new_value.map(val => new ElementDefinitionSlicingDiscriminator(val)) : [new ElementDefinitionSlicingDiscriminator(new_value)];
	}
	...
	toJSON () {
		return Object.assign(super.toJSON(), {
			discriminator: this.__discriminator && this.__discriminator.map(v => v.toJSON()),
