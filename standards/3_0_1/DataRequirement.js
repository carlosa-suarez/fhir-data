...
class DataRequirement extends Element {
	...
	set codeFilter (new_value) {
		...
		this.__codeFilter = Array.isArray(new_value) ? new_value.map(val => new DataRequirementCodeFilter(val)) : [new DataRequirementCodeFilter(new_value)];
	}
	...
	set dateFilter (new_value) {
		...
		this.__dateFilter = Array.isArray(new_value) ? new_value.map(val => new DataRequirementDateFilter(val)) : [new DataRequirementDateFilter(new_value)];
	}
