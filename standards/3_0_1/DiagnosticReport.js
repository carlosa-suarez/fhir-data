...
class DiagnosticReport extends DomainResource {
	...
	set performer (new_value) {
		...
		this.__performer = Array.isArray(new_value) ? new_value.map(val => new DiagnosticReportPerformer(val)) : [new DiagnosticReportPerformer(new_value)];
	}
	...
	set imagingStudy (new_value) {
		const Reference = require('./Reference');
		this.__imagingStudy = Array.isArray(new_value) ? new_value.map(val => new Reference(val)) : [new Reference(new_value)];
	}
	...
	set image (new_value) {
		...
		this.__image = Array.isArray(new_value) ? new_value.map(val => new DiagnosticReportImage(val)) : [new DiagnosticReportImage(new_value)];
	}
	...
	set codedDiagnosis (new_value) {
		const CodeableConcept = require('./CodeableConcept');
		this.__codedDiagnosis = Array.isArray(new_value) ? new_value.map(val => new CodeableConcept(val)) : [new CodeableConcept(new_value)];
	}
	...
	set presentedForm (new_value) {
		...
		this.__presentedForm = Array.isArray(new_value) ? new_value.map(val => new Attachment(val)) : [new Attachment(new_value)];
	}
