...
class ClinicalImpression extends DomainResource {
	...
	set investigation (new_value) {
		...
		this.__investigation = Array.isArray(new_value) ? new_value.map(val => new ClinicalImpressionInvestigation(val)) : [new ClinicalImpressionInvestigation(new_value)];
	}
	...
	set finding (new_value) {
		...
		this.__finding = Array.isArray(new_value) ? new_value.map(val => new ClinicalImpressionFinding(val)) : [new ClinicalImpressionFinding(new_value)];
	}
	...
	set prognosisCodeableConcept (new_value) {
		const CodeableConcept = require('./CodeableConcept');
		this.__prognosisCodeableConcept = Array.isArray(new_value) ? new_value.map(val => new CodeableConcept(val)) : [new CodeableConcept(new_value)];
	}
	...
	set prognosisReference (new_value) {
		const Reference = require('./Reference');
		this.__prognosisReference = Array.isArray(new_value) ? new_value.map(val => new Reference(val)) : [new Reference(new_value)];
	}
