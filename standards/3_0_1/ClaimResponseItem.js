...
class ClaimResponseItem extends BackboneElement {
	...
	set adjudication (new_value) {
		...
		this.__adjudication = Array.isArray(new_value) ? new_value.map(val => new ClaimResponseItemAdjudication(val)) : [new ClaimResponseItemAdjudication(new_value)];
	}
	...
	set detail (new_value) {
		...
		this.__detail = Array.isArray(new_value) ? new_value.map(val => new ClaimResponseItemDetail(val)) : [new ClaimResponseItemDetail(new_value)];
	}
