...
class TestScriptSetupActionAssertRuleset extends BackboneElement {
	...
	set rule (new_value) {
		...
		this.__rule = Array.isArray(new_value) ? new_value.map(val => new TestScriptSetupActionAssertRulesetRule(val)) : [new TestScriptSetupActionAssertRulesetRule(new_value)];
	}
