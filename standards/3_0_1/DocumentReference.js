...
class DocumentReference extends DomainResource {
	...
	set relatesTo (new_value) {
		...
		this.__relatesTo = Array.isArray(new_value) ? new_value.map(val => new DocumentReferenceRelatesTo(val)) : [new DocumentReferenceRelatesTo(new_value)];
	}
	...
	set securityLabel (new_value) {
		const CodeableConcept = require('./CodeableConcept');
		this.__securityLabel = Array.isArray(new_value) ? new_value.map(val => new CodeableConcept(val)) : [new CodeableConcept(new_value)];
	}
	...
	set content (new_value) {
		...
		this.__content = Array.isArray(new_value) ? new_value.map(val => new DocumentReferenceContent(val)) : [new DocumentReferenceContent(new_value)];
	}
