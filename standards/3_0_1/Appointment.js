...
class Appointment extends DomainResource {
	...
	set indication (new_value) {
		...
		this.__indication = Array.isArray(new_value) ? new_value.map(val => new Reference(val)) : [new Reference(new_value)];
	}
	...
	set participant (new_value) {
		...
		this.__participant = Array.isArray(new_value) ? new_value.map(val => new AppointmentParticipant(val)) : [new AppointmentParticipant(new_value)];
	}
