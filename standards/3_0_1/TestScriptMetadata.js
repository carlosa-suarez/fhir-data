...
class TestScriptMetadata extends BackboneElement {
	...
	set link (new_value) {
		...
		this.__link = Array.isArray(new_value) ? new_value.map(val => new TestScriptMetadataLink(val)) : [new TestScriptMetadataLink(new_value)];
	}
	...
	set capability (new_value) {
		...
		this.__capability = Array.isArray(new_value) ? new_value.map(val => new TestScriptMetadataCapability(val)) : [new TestScriptMetadataCapability(new_value)];
	}
