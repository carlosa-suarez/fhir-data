...
class Composition extends DomainResource {
	...
	set attester (new_value) {
		...
		this.__attester = Array.isArray(new_value) ? new_value.map(val => new CompositionAttester(val)) : [new CompositionAttester(new_value)];
	}
	...
	set relatesTo (new_value) {
		...
		this.__relatesTo = Array.isArray(new_value) ? new_value.map(val => new CompositionRelatesTo(val)) : [new CompositionRelatesTo(new_value)];
	}
	...
	set event (new_value) {
		...
		this.__event = Array.isArray(new_value) ? new_value.map(val => new CompositionEvent(val)) : [new CompositionEvent(new_value)];
	}
	...
	set section (new_value) {
		...
		this.__section = Array.isArray(new_value) ? new_value.map(val => new CompositionSection(val)) : [new CompositionSection(new_value)];
	}
