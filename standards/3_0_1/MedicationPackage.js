...
class MedicationPackage extends BackboneElement {
	...
	set content (new_value) {
		...
		this.__content = Array.isArray(new_value) ? new_value.map(val => new MedicationPackageContent(val)) : [new MedicationPackageContent(new_value)];
	}
	...
	set batch (new_value) {
		...
		this.__batch = Array.isArray(new_value) ? new_value.map(val => new MedicationPackageBatch(val)) : [new MedicationPackageBatch(new_value)];
	}
