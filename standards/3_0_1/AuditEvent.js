...
class AuditEvent extends DomainResource {
	...
	set purposeOfEvent (new_value) {
		...
		this.__purposeOfEvent = Array.isArray(new_value) ? new_value.map(val => new CodeableConcept(val)) : [new CodeableConcept(new_value)];
	}
	...
	set agent (new_value) {
		...
		this.__agent = Array.isArray(new_value) ? new_value.map(val => new AuditEventAgent(val)) : [new AuditEventAgent(new_value)];
	}
	...
	set entity (new_value) {
		...
		this.__entity = Array.isArray(new_value) ? new_value.map(val => new AuditEventEntity(val)) : [new AuditEventEntity(new_value)];
	}
