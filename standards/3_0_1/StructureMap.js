...
class StructureMap extends DomainResource {
	...
	set structure (new_value) {
		...
		this.__structure = Array.isArray(new_value) ? new_value.map(val => new StructureMapStructure(val)) : [new StructureMapStructure(new_value)];
	}
	...
	set group (new_value) {
		...
		this.__group = Array.isArray(new_value) ? new_value.map(val => new StructureMapGroup(val)) : [new StructureMapGroup(new_value)];
	}
