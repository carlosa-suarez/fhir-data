...
class CarePlanActivity extends BackboneElement {
	...
	set outcomeCodeableConcept (new_value) {
		...
		this.__outcomeCodeableConcept = Array.isArray(new_value) ? new_value.map(val => new CodeableConcept(val)) : [new CodeableConcept(new_value)];
	}
	...
	set outcomeReference (new_value) {
		...
		this.__outcomeReference = Array.isArray(new_value) ? new_value.map(val => new Reference(val)) : [new Reference(new_value)];
	}
	...
	set progress (new_value) {
		...
		this.__progress = Array.isArray(new_value) ? new_value.map(val => new Annotation(val)) : [new Annotation(new_value)];
	}
