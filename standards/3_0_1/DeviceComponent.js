...
class DeviceComponent extends DomainResource {
	...
	set operationalStatus (new_value) {
		const CodeableConcept = require('./CodeableConcept');
		this.__operationalStatus = Array.isArray(new_value) ? new_value.map(val => new CodeableConcept(val)) : [new CodeableConcept(new_value)];
	}
	...
	set productionSpecification (new_value) {
		...
		this.__productionSpecification = Array.isArray(new_value) ? new_value.map(val => new DeviceComponentProductionSpecification(val)) : [new DeviceComponentProductionSpecification(new_value)];
	}
