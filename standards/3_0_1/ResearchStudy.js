...
class ResearchStudy extends DomainResource {
	...
	set focus (new_value) {
		const CodeableConcept = require('./CodeableConcept');
		this.__focus = Array.isArray(new_value) ? new_value.map(val => new CodeableConcept(val)) : [new CodeableConcept(new_value)];
	}
	...
	set keyword (new_value) {
		const CodeableConcept = require('./CodeableConcept');
		this.__keyword = Array.isArray(new_value) ? new_value.map(val => new CodeableConcept(val)) : [new CodeableConcept(new_value)];
	}
	...
	set enrollment (new_value) {
		const Reference = require('./Reference');
		this.__enrollment = Array.isArray(new_value) ? new_value.map(val => new Reference(val)) : [new Reference(new_value)];
	}
	...
	set arm (new_value) {
		...
		this.__arm = Array.isArray(new_value) ? new_value.map(val => new ResearchStudyArm(val)) : [new ResearchStudyArm(new_value)];
	}
