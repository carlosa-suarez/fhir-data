...
class AdverseEvent extends DomainResource {
	...
	set suspectEntity (new_value) {
		...
		this.__suspectEntity = Array.isArray(new_value) ? new_value.map(val => new AdverseEventSuspectEntity(val)) : [new AdverseEventSuspectEntity(new_value)];
	}
	...
	set subjectMedicalHistory (new_value) {
		const Reference = require('./Reference');
		this.__subjectMedicalHistory = Array.isArray(new_value) ? new_value.map(val => new Reference(val)) : [new Reference(new_value)];
	}
	...
	set referenceDocument (new_value) {
		const Reference = require('./Reference');
		this.__referenceDocument = Array.isArray(new_value) ? new_value.map(val => new Reference(val)) : [new Reference(new_value)];
	}
