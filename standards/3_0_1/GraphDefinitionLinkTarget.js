...
class GraphDefinitionLinkTarget extends BackboneElement {
	...
	set compartment (new_value) {
		...
		this.__compartment = Array.isArray(new_value) ? new_value.map(val => new GraphDefinitionLinkTargetCompartment(val)) : [new GraphDefinitionLinkTargetCompartment(new_value)];
	}
