...
class NutritionOrderEnteralFormula extends BackboneElement {
	...
	set administration (new_value) {
		...
		this.__administration = Array.isArray(new_value) ? new_value.map(val => new NutritionOrderEnteralFormulaAdministration(val)) : [new NutritionOrderEnteralFormulaAdministration(new_value)];
	}
