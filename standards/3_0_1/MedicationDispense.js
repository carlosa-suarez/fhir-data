...
class MedicationDispense extends DomainResource {
	...
	set supportingInformation (new_value) {
		const Reference = require('./Reference');
		this.__supportingInformation = Array.isArray(new_value) ? new_value.map(val => new Reference(val)) : [new Reference(new_value)];
	}
	...
	set performer (new_value) {
		...
		this.__performer = Array.isArray(new_value) ? new_value.map(val => new MedicationDispensePerformer(val)) : [new MedicationDispensePerformer(new_value)];
	}
	...
	set authorizingPrescription (new_value) {
		const Reference = require('./Reference');
		this.__authorizingPrescription = Array.isArray(new_value) ? new_value.map(val => new Reference(val)) : [new Reference(new_value)];
	}
	...
	set dosageInstruction (new_value) {
		...
		this.__dosageInstruction = Array.isArray(new_value) ? new_value.map(val => new Dosage(val)) : [new Dosage(new_value)];
	}
	...
	set detectedIssue (new_value) {
		const Reference = require('./Reference');
		this.__detectedIssue = Array.isArray(new_value) ? new_value.map(val => new Reference(val)) : [new Reference(new_value)];
	}
	...
	set eventHistory (new_value) {
		const Reference = require('./Reference');
		this.__eventHistory = Array.isArray(new_value) ? new_value.map(val => new Reference(val)) : [new Reference(new_value)];
	}
