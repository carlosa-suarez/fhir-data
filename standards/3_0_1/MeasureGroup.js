...
class MeasureGroup extends BackboneElement {
	...
	set population (new_value) {
		...
		this.__population = Array.isArray(new_value) ? new_value.map(val => new MeasureGroupPopulation(val)) : [new MeasureGroupPopulation(new_value)];
	}
	...
	set stratifier (new_value) {
		...
		this.__stratifier = Array.isArray(new_value) ? new_value.map(val => new MeasureGroupStratifier(val)) : [new MeasureGroupStratifier(new_value)];
	}
