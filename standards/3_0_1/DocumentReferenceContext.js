...
class DocumentReferenceContext extends BackboneElement {
	...
	set event (new_value) {
		...
		this.__event = Array.isArray(new_value) ? new_value.map(val => new CodeableConcept(val)) : [new CodeableConcept(new_value)];
	}
	...
	set related (new_value) {
		...
		this.__related = Array.isArray(new_value) ? new_value.map(val => new DocumentReferenceContextRelated(val)) : [new DocumentReferenceContextRelated(new_value)];
	}
