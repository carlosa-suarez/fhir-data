...
class PlanDefinitionGoal extends BackboneElement {
	...
	set addresses (new_value) {
		const CodeableConcept = require('./CodeableConcept');
		this.__addresses = Array.isArray(new_value) ? new_value.map(val => new CodeableConcept(val)) : [new CodeableConcept(new_value)];
	}
	...
	set target (new_value) {
		...
		this.__target = Array.isArray(new_value) ? new_value.map(val => new PlanDefinitionGoalTarget(val)) : [new PlanDefinitionGoalTarget(new_value)];
	}
