...
class MeasureReportGroupStratifier extends BackboneElement {
	...
	set stratum (new_value) {
		...
		this.__stratum = Array.isArray(new_value) ? new_value.map(val => new MeasureReportGroupStratifierStratum(val)) : [new MeasureReportGroupStratifierStratum(new_value)];
	}
