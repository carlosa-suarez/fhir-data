...
class PractitionerRole extends DomainResource {
	...
	set healthcareService (new_value) {
		const Reference = require('./Reference');
		this.__healthcareService = Array.isArray(new_value) ? new_value.map(val => new Reference(val)) : [new Reference(new_value)];
	}
	...
	set availableTime (new_value) {
		...
		this.__availableTime = Array.isArray(new_value) ? new_value.map(val => new PractitionerRoleAvailableTime(val)) : [new PractitionerRoleAvailableTime(new_value)];
	}
	...
	set notAvailable (new_value) {
		...
		this.__notAvailable = Array.isArray(new_value) ? new_value.map(val => new PractitionerRoleNotAvailable(val)) : [new PractitionerRoleNotAvailable(new_value)];
	}
