...
class CapabilityStatementMessaging extends BackboneElement {
	...
	set endpoint (new_value) {
		...
		this.__endpoint = Array.isArray(new_value) ? new_value.map(val => new CapabilityStatementMessagingEndpoint(val)) : [new CapabilityStatementMessagingEndpoint(new_value)];
	}
	...
	set supportedMessage (new_value) {
		...
		this.__supportedMessage = Array.isArray(new_value) ? new_value.map(val => new CapabilityStatementMessagingSupportedMessage(val)) : [new CapabilityStatementMessagingSupportedMessage(new_value)];
	}
	...
	set event (new_value) {
		...
		this.__event = Array.isArray(new_value) ? new_value.map(val => new CapabilityStatementMessagingEvent(val)) : [new CapabilityStatementMessagingEvent(new_value)];
	}
