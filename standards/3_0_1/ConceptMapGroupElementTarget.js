...
class ConceptMapGroupElementTarget extends BackboneElement {
	...
	set dependsOn (new_value) {
		...
		this.__dependsOn = Array.isArray(new_value) ? new_value.map(val => new ConceptMapGroupElementTargetDependsOn(val)) : [new ConceptMapGroupElementTargetDependsOn(new_value)];
	}
