...
class CapabilityStatementRestSecurity extends BackboneElement {
	...
	set service (new_value) {
		...
		this.__service = Array.isArray(new_value) ? new_value.map(val => new CodeableConcept(val)) : [new CodeableConcept(new_value)];
	}
	...
	set certificate (new_value) {
		...
		this.__certificate = Array.isArray(new_value) ? new_value.map(val => new CapabilityStatementRestSecurityCertificate(val)) : [new CapabilityStatementRestSecurityCertificate(new_value)];
	}
