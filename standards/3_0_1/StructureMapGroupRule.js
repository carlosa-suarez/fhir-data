...
class StructureMapGroupRule extends BackboneElement {
	...
	set source (new_value) {
		...
		this.__source = Array.isArray(new_value) ? new_value.map(val => new StructureMapGroupRuleSource(val)) : [new StructureMapGroupRuleSource(new_value)];
	}
	...
	set target (new_value) {
		...
		this.__target = Array.isArray(new_value) ? new_value.map(val => new StructureMapGroupRuleTarget(val)) : [new StructureMapGroupRuleTarget(new_value)];
	}
	...
	set dependent (new_value) {
		...
		this.__dependent = Array.isArray(new_value) ? new_value.map(val => new StructureMapGroupRuleDependent(val)) : [new StructureMapGroupRuleDependent(new_value)];
	}
