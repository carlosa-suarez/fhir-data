...
class EligibilityResponseInsuranceBenefitBalance extends BackboneElement {
	...
	set financial (new_value) {
		...
		this.__financial = Array.isArray(new_value) ? new_value.map(val => new EligibilityResponseInsuranceBenefitBalanceFinancial(val)) : [new EligibilityResponseInsuranceBenefitBalanceFinancial(new_value)];
	}
