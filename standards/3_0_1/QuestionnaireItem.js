...
class QuestionnaireItem extends BackboneElement {
	...
	set enableWhen (new_value) {
		...
		this.__enableWhen = Array.isArray(new_value) ? new_value.map(val => new QuestionnaireItemEnableWhen(val)) : [new QuestionnaireItemEnableWhen(new_value)];
	}
	...
	set option (new_value) {
		...
		this.__option = Array.isArray(new_value) ? new_value.map(val => new QuestionnaireItemOption(val)) : [new QuestionnaireItemOption(new_value)];
	}
