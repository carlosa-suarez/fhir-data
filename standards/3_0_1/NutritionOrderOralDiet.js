...
class NutritionOrderOralDiet extends BackboneElement {
	...
	set nutrient (new_value) {
		...
		this.__nutrient = Array.isArray(new_value) ? new_value.map(val => new NutritionOrderOralDietNutrient(val)) : [new NutritionOrderOralDietNutrient(new_value)];
	}
	...
	set texture (new_value) {
		...
		this.__texture = Array.isArray(new_value) ? new_value.map(val => new NutritionOrderOralDietTexture(val)) : [new NutritionOrderOralDietTexture(new_value)];
	}
	...
	set fluidConsistencyType (new_value) {
		const CodeableConcept = require('./CodeableConcept');
		this.__fluidConsistencyType = Array.isArray(new_value) ? new_value.map(val => new CodeableConcept(val)) : [new CodeableConcept(new_value)];
	}
