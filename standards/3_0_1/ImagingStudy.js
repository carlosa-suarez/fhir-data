...
class ImagingStudy extends DomainResource {
	...
	set interpreter (new_value) {
		const Reference = require('./Reference');
		this.__interpreter = Array.isArray(new_value) ? new_value.map(val => new Reference(val)) : [new Reference(new_value)];
	}
	...
	set procedureReference (new_value) {
		const Reference = require('./Reference');
		this.__procedureReference = Array.isArray(new_value) ? new_value.map(val => new Reference(val)) : [new Reference(new_value)];
	}
	...
	set procedureCode (new_value) {
		...
		this.__procedureCode = Array.isArray(new_value) ? new_value.map(val => new CodeableConcept(val)) : [new CodeableConcept(new_value)];
	}
	...
	set series (new_value) {
		...
		this.__series = Array.isArray(new_value) ? new_value.map(val => new ImagingStudySeries(val)) : [new ImagingStudySeries(new_value)];
	}
