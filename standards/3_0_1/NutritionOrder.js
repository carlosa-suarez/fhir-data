...
class NutritionOrder extends DomainResource {
	...
	set allergyIntolerance (new_value) {
		const Reference = require('./Reference');
		this.__allergyIntolerance = Array.isArray(new_value) ? new_value.map(val => new Reference(val)) : [new Reference(new_value)];
	}
	...
	set foodPreferenceModifier (new_value) {
		...
		this.__foodPreferenceModifier = Array.isArray(new_value) ? new_value.map(val => new CodeableConcept(val)) : [new CodeableConcept(new_value)];
	}
	...
	set excludeFoodModifier (new_value) {
		const CodeableConcept = require('./CodeableConcept');
		this.__excludeFoodModifier = Array.isArray(new_value) ? new_value.map(val => new CodeableConcept(val)) : [new CodeableConcept(new_value)];
	}
	...
	set supplement (new_value) {
		...
		this.__supplement = Array.isArray(new_value) ? new_value.map(val => new NutritionOrderSupplement(val)) : [new NutritionOrderSupplement(new_value)];
	}
