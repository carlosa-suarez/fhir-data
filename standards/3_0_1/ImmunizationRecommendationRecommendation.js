...
class ImmunizationRecommendationRecommendation extends BackboneElement {
	...
	set targetDisease (new_value) {
		const CodeableConcept = require('./CodeableConcept');
		this.__targetDisease = new CodeableConcept(new_value);
	}
	...
	set dateCriterion (new_value) {
		...
		this.__dateCriterion = Array.isArray(new_value) ? new_value.map(val => new ImmunizationRecommendationRecommendationDateCriterion(val)) : [new ImmunizationRecommendationRecommendationDateCriterion(new_value)];
	}
	...
	set supportingImmunization (new_value) {
		...
		this.__supportingImmunization = Array.isArray(new_value) ? new_value.map(val => new Reference(val)) : [new Reference(new_value)];
	}
	...
	set supportingPatientInformation (new_value) {
		const Reference = require('./Reference');
		this.__supportingPatientInformation = Array.isArray(new_value) ? new_value.map(val => new Reference(val)) : [new Reference(new_value)];
	}
	...
	toJSON () {
		return Object.assign(super.toJSON(), {
			...
			targetDisease: this.__targetDisease && this.__targetDisease.toJSON(),
			...
			supportingPatientInformation: this.__supportingPatientInformation && this.__supportingPatientInformation.map(v => v.toJSON())
		});
	}
}
