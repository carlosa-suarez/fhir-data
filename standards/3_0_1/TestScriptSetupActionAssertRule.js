...
class TestScriptSetupActionAssertRule extends BackboneElement {
	...
	set param (new_value) {
		...
		this.__param = Array.isArray(new_value) ? new_value.map(val => new TestScriptSetupActionAssertRuleParam(val)) : [new TestScriptSetupActionAssertRuleParam(new_value)];
	}
