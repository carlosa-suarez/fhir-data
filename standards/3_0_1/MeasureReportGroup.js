...
class MeasureReportGroup extends BackboneElement {
	...
	set population (new_value) {
		...
		this.__population = Array.isArray(new_value) ? new_value.map(val => new MeasureReportGroupPopulation(val)) : [new MeasureReportGroupPopulation(new_value)];
	}
	...
	set stratifier (new_value) {
		...
		this.__stratifier = Array.isArray(new_value) ? new_value.map(val => new MeasureReportGroupStratifier(val)) : [new MeasureReportGroupStratifier(new_value)];
	}
