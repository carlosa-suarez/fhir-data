...
class CareTeam extends DomainResource {
	...
	set participant (new_value) {
		...
		this.__participant = Array.isArray(new_value) ? new_value.map(val => new CareTeamParticipant(val)) : [new CareTeamParticipant(new_value)];
	}
	...
	set managingOrganization (new_value) {
		const Reference = require('./Reference');
		this.__managingOrganization = Array.isArray(new_value) ? new_value.map(val => new Reference(val)) : [new Reference(new_value)];
	}
