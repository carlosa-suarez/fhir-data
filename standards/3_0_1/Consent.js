...
class Consent extends DomainResource {
	...
	set consentingParty (new_value) {
		const Reference = require('./Reference');
		this.__consentingParty = Array.isArray(new_value) ? new_value.map(val => new Reference(val)) : [new Reference(new_value)];
	}
	...
	set actor (new_value) {
		...
		this.__actor = Array.isArray(new_value) ? new_value.map(val => new ConsentActor(val)) : [new ConsentActor(new_value)];
	}
	...
	set organization (new_value) {
		const Reference = require('./Reference');
		this.__organization = Array.isArray(new_value) ? new_value.map(val => new Reference(val)) : [new Reference(new_value)];
	}
	...
	set policy (new_value) {
		...
		this.__policy = Array.isArray(new_value) ? new_value.map(val => new ConsentPolicy(val)) : [new ConsentPolicy(new_value)];
	}
	...
	set except (new_value) {
		...
		this.__except = Array.isArray(new_value) ? new_value.map(val => new ConsentExcept(val)) : [new ConsentExcept(new_value)];
	}
