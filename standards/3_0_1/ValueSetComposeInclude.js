...
class ValueSetComposeInclude extends BackboneElement {
	...
	set concept (new_value) {
		...
		this.__concept = Array.isArray(new_value) ? new_value.map(val => new ValueSetComposeIncludeConcept(val)) : [new ValueSetComposeIncludeConcept(new_value)];
	}
	...
	set filter (new_value) {
		...
		this.__filter = Array.isArray(new_value) ? new_value.map(val => new ValueSetComposeIncludeFilter(val)) : [new ValueSetComposeIncludeFilter(new_value)];
	}
	...
	set valueSet (new_value) {
		...
		this.__valueSet = Array.isArray(new_value) ? new_value : [new_value];
	}
