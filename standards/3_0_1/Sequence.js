...
class Sequence extends DomainResource {
	...
	set variant (new_value) {
		...
		this.__variant = Array.isArray(new_value) ? new_value.map(val => new SequenceVariant(val)) : [new SequenceVariant(new_value)];
	}
	...
	set quality (new_value) {
		...
		this.__quality = Array.isArray(new_value) ? new_value.map(val => new SequenceQuality(val)) : [new SequenceQuality(new_value)];
	}
	...
	set repository (new_value) {
		...
		this.__repository = Array.isArray(new_value) ? new_value.map(val => new SequenceRepository(val)) : [new SequenceRepository(new_value)];
	}
