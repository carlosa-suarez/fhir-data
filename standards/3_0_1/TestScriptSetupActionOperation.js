...
class TestScriptSetupActionOperation extends BackboneElement {
	...
	set requestHeader (new_value) {
		...
		this.__requestHeader = Array.isArray(new_value) ? new_value.map(val => new TestScriptSetupActionOperationRequestHeader(val)) : [new TestScriptSetupActionOperationRequestHeader(new_value)];
	}
