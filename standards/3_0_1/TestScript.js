...
class TestScript extends DomainResource {
	...
	set origin (new_value) {
		...
		this.__origin = Array.isArray(new_value) ? new_value.map(val => new TestScriptOrigin(val)) : [new TestScriptOrigin(new_value)];
	}
	...
	set destination (new_value) {
		...
		this.__destination = Array.isArray(new_value) ? new_value.map(val => new TestScriptDestination(val)) : [new TestScriptDestination(new_value)];
	}
	...
	set fixture (new_value) {
		...
		this.__fixture = Array.isArray(new_value) ? new_value.map(val => new TestScriptFixture(val)) : [new TestScriptFixture(new_value)];
	}
	...
	set variable (new_value) {
		...
		this.__variable = Array.isArray(new_value) ? new_value.map(val => new TestScriptVariable(val)) : [new TestScriptVariable(new_value)];
	}
	...
	set rule (new_value) {
		...
		this.__rule = Array.isArray(new_value) ? new_value.map(val => new TestScriptRule(val)) : [new TestScriptRule(new_value)];
	}
	...
	set ruleset (new_value) {
		...
		this.__ruleset = Array.isArray(new_value) ? new_value.map(val => new TestScriptRuleset(val)) : [new TestScriptRuleset(new_value)];
	}
	...
	set test (new_value) {
		...
		this.__test = Array.isArray(new_value) ? new_value.map(val => new TestScriptTest(val)) : [new TestScriptTest(new_value)];
	}
