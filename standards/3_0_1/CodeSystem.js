...
class CodeSystem extends DomainResource {
	...
	set filter (new_value) {
		...
		this.__filter = Array.isArray(new_value) ? new_value.map(val => new CodeSystemFilter(val)) : [new CodeSystemFilter(new_value)];
	}
	...
	set property (new_value) {
		...
		this.__property = Array.isArray(new_value) ? new_value.map(val => new CodeSystemProperty(val)) : [new CodeSystemProperty(new_value)];
	}
	...
	set concept (new_value) {
		...
		this.__concept = Array.isArray(new_value) ? new_value.map(val => new CodeSystemConcept(val)) : [new CodeSystemConcept(new_value)];
	}
